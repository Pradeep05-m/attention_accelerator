    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (q_wr_valid && q_wr_ready) begin
                ...
                $display("[%0t] DEBUG-QGRP: wrote head into fill_idx=%0d", $time, fill_idx);
                if (fill_idx == GROUP_SIZE-1) begin
                    fill_idx <= 0;
                    group_ready <= 1'b1;
                    $display("[%0t] DEBUG-QGRP: GROUP_READY asserted (4 heads buffered)", $time);
                end else fill_idx <= fill_idx + 1'b1;
            end else if (q_wr_valid && !q_wr_ready) begin
                $display("[%0t] DEBUG-QGRP: Q WRITE REJECTED (q_wr_ready=0, group_ready=%0d group_active=%0d)", $time, group_ready, group_active);
            end
            if (q_req && group_ready && !group_active) begin
                group_active <= 1'b1;
                q_ack <= 1'b1;
                $display("[%0t] DEBUG-QGRP: q_req -> q_ack fired", $time);
            end else if (q_req) begin
                $display("[%0t] DEBUG-QGRP: q_req asserted but NOT acked (group_ready=%0d group_active=%0d)", $time, group_ready, group_active);
            end
            if (q_release) begin
                fill_idx <= 0;
                group_ready <= 1'b0;
